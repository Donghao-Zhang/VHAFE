...
class SimBuffer(object):
    def __init__(self, args, obs_space, share_obs_space, act_space):
        ...
        self.value_preds = np.zeros((self.episode_length + 1, self.n_rollout_threads, args.o2_number, 1), dtype=np.float32)
        self.returns = np.zeros((self.episode_length + 1, self.n_rollout_threads, args.o2_number, 1), dtype=np.float32)

        act_shape = get_shape_from_act_space(act_space)

        self.actions = np.zeros((self.episode_length, self.n_rollout_threads, args.o2_number, act_shape), dtype=np.float32)
        self.action_log_probs = np.zeros((self.episode_length, self.n_rollout_threads, args.o2_number, act_shape), dtype=np.float32)
        self.rewards = np.zeros((self.episode_length, self.n_rollout_threads, 1), dtype=np.float32)
        self.action_masks = np.ones((self.episode_length, self.n_rollout_threads, args.o2_number, 1), dtype=np.float32)
    ...
    def insert(self, share_obs, obs, actions, action_log_probs,
        ...
        self.value_preds[self.step] = value_preds.copy()
        self.rewards[self.step] = rewards.copy()
        self.action_masks[self.step] = masks.copy()
    ...
    def compute_returns(self, next_value, value_normalizer=None):
        ...
        # gae = 0
        for step in reversed(range(self.rewards.shape[0])):
            self.returns[step] = (self.rewards[step][:, None, :] +
                                  self.gamma * value_normalizer.denormalize(self.value_preds[step + 1]) -
                                  value_normalizer.denormalize(self.value_preds[step])) * self.action_masks[step]
    ...
    def feed_forward_generator(self, advantages, num_mini_batch=None, mini_batch_size=None):
        ...
        action_masks = self.action_masks.reshape(-1, *self.actions.shape[2:])
        # if self.available_actions is not None:
        #     available_actions = self.available_actions[:-1].reshape(-1, self.available_actions.shape[-1])
        ...
        for indices in sampler:
            ...
            obs_batch = obs[indices]
            actions_batch = actions[indices]
            actions_batch_masks = action_masks[indices]
            # if self.available_actions is not None:
            #     available_actions_batch = available_actions[indices]
            # else:
            available_actions_batch = None
            ...
            yield share_obs_batch, obs_batch, actions_batch, actions_batch_masks, value_preds_batch, return_batch, masks_batch, active_masks_batch, old_action_log_probs_batch, adv_targ, available_actions_batch
