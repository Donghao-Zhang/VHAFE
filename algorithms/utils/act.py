from .distributions import Bernoulli, Categorical, DiagGaussian, SimCategorical
import torch
...
class ACTLayer(nn.Module):
    ...
    def __init__(self, action_space, inputs_dim, use_orthogonal, gain):
        ...
        if isinstance(action_space, int):
            self.action_out = SimCategorical(inputs_dim, action_space, use_orthogonal, gain)
        ...
        if self.mixed_action:
            ...
            if active_masks is not None:
                if active_masks.sum() == 0:
                    dist_entropy = (action_logits.entropy()*active_masks.squeeze(-1)).sum()
                else:
